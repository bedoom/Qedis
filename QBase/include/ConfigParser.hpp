...
public:
    ...
    {
        std::cout << "//////////////////"<< std::endl;
        std::map<std::string, std::vector<std::string> >::const_iterator it = data_.begin();
        while (it != data_.end())
        {
            std::cout << it->first << ": ";
...
template<>
inline const char* ConfigParser::_ToType(const std::string& data) const
{
    return data.c_str();
}

template<>
inline std::string ConfigParser::_ToType(const std::string& data) const
{
    return data;
}

template<typename T>
T ConfigParser::GetData(const char* key, const T* default_) const
{
